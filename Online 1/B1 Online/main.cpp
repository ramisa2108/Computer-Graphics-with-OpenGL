...
void drawSS()
{
	...
	glPushMatrix();
	{
		...
		glBegin(GL_LINES);
		{
			glVertex3f(0, 0, 0);
			//glVertex3f(10, 10, 0);
			glVertex3f(-5 * wheel_radius * pi / 6.0  * cos(rotation_angle * pi / 180), -5 * wheel_radius * pi / 6.0  * sin(rotation_angle * pi / 180), 0);
		}
		glEnd();
	}
	glPopMatrix();
	...
	// 	glBegin(GL_QUADS);{
	// 		double dir_x = wheel_center.x - 10.0 * wheel_radius * pi / 6.0  * cos(rotation_angle * pi / 180); 
	// 		double dir_y = wheel_center.y - 10.0 * wheel_radius * pi / 6.0  * sin(rotation_angle * pi / 180);
	// 		printf("%f %f %f\n", wheel_center.x, wheel_center.y, wheel_center.z);
